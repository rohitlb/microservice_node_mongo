...
var Eureka = function () {
  function Eureka() {
    ...
    this.config = (0, _deepmerge2.default)(_defaultConfig2.default, getYaml(_path2.default.join(cwd, filename + '.yml')));
    this.config = (0, _deepmerge2.default)(this.config, getYaml(_path2.default.join(cwd, filename + '-' + env + '.yml')));
    ...
    if (this.amazonDataCenter) {
      this.metadataClient = new _AwsMetadata2.default({
        logger: this.logger
      });
    }
    ...
    this.buildEurekaUrl(function (eurekaUrl) {
      _request2.default.put({
        ...
        if (!error && response.statusCode === 200) {
          ...
          _this6.logger.warn('eureka heartbeat FAILED, will retry.', 'status: ' + (response ? response.statusCode : 'unknown') + ' body: ' + body);
        }
      });
    });
  };
  ...
  */
  ...
  Eureka.prototype.addInstanceMetadata = function addInstanceMetadata() {
    ...
    this.metadataClient.fetchMetadata(function (metadataResult) {
      _this9.config.instance.dataCenterInfo.metadata = (0, _deepmerge2.default)(_this9.config.instance.dataCenterInfo.metadata, metadataResult);
      ...
      if (_this9.config.instance.statusPageUrl) {
        var statusPageUrl = _this9.config.instance.statusPageUrl;

        var replacedUrl = statusPageUrl.replace('__HOST__', metadataResult['public-hostname']);
        _this9.config.instance.statusPageUrl = replacedUrl;
      }
      if (_this9.config.instance.healthCheckUrl) {
        var healthCheckUrl = _this9.config.instance.healthCheckUrl;

        var replacedUrl = healthCheckUrl.replace('__HOST__', metadataResult['public-hostname']);
        _this9.config.instance.healthCheckUrl = replacedUrl;
      }
  ...
  */
  ...
  Eureka.prototype.locateEurekaHostUsingDns = function locateEurekaHostUsingDns() {
    ...
    _dns2.default.resolveTxt('txt.' + ec2Region + '.' + host, function (err, addresses) {
      if (err) {
        throw new Error('Error resolving eureka server list for region [' + ec2Region + '] using DNS: [' + err + ']');
      }
      ...
      _dns2.default.resolveTxt('txt.' + addresses[0][random], function (resolveErr, results) {
        ...
        callback((_ref = []).concat.apply(_ref, results).shift());
      });
    });
  };
